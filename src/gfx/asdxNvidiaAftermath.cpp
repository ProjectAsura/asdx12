...
#include <direct.h>
#include <d3d12.h>
#include "../external/NVIDIA_Nsight_Aftermath_SDK/include/GFSDK_Aftermath.h"
...
private:
    ...
    (
        ...
        void*                               pUserData,
        PFN_GFSDK_Aftermath_ResolveMarker   pResolverMarker
    )
    {
        auto tracker = reinterpret_cast<GpuCrashTracker*>(pUserData);
        if (tracker != nullptr)
        { tracker->OnResolveMarker(pMakerData, markerDataSize); }
    }
            ...
            if (GFSDK_Aftermath_SUCCEED(ret))
            {
                ...
                if (GFSDK_Aftermath_SUCCEED(ret))
                {
                    for(size_t i=0; i<shaderInfos.size(); ++i)
                    {
                        ...
                        OutputLog("  DebugInfo Uid : %llu", info.shaderDebugInfoUid);
