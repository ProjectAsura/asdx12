...
//      角度を求めます.
//-----------------------------------------------------------------------------
inline float GetAngle( float sin, float cos )
{
...
{ return m_View.GetBasisX(); }

//-----------------------------------------------------------------------------
//      カメラ基底ベクトルのY軸を取得します.
//-----------------------------------------------------------------------------
asdx::Vector3 Camera::GetAxisY() const
{ return m_View.GetBasisY(); }

//-----------------------------------------------------------------------------
//      カメラ基底ベクトルのZ軸を取得します.
//-----------------------------------------------------------------------------
asdx::Vector3 Camera::GetAxisZ() const
{ return m_View.GetBasisZ(); }

//-----------------------------------------------------------------------------
//      パラメータを取得します.
//-----------------------------------------------------------------------------
const Camera::Param& Camera::GetParam() const
{ return m_Param; }

//-----------------------------------------------------------------------------
//      パラメータを設定します.
//-----------------------------------------------------------------------------
void Camera::SetParam(const Param& param)
{ m_Param = param; }
...
void Camera::UpdateByEvent(const CameraEvent& camEvent)
{
    ...
    constexpr auto limitAngle = ToRadian(89.9f);
    ...
    if (camEvent.Flags & CameraEvent::EVENT_ROTATE)
    {
        ...
        // 90度制限.
        if (m_Param.Rotate.y > limitAngle)
        { m_Param.Rotate.y = limitAngle; }
        if (m_Param.Rotate.y < -limitAngle)
        { m_Param.Rotate.y = -limitAngle; }
    ...
    {
        // 視線ベクトルを作成.
        auto dir  = m_Param.Position - m_Param.Target;
        auto dist = Vector3::Distance(m_Param.Position, m_Param.Target);
    ...
    if (camEvent.Flags & CameraEvent::EVENT_PANTILT)
    {
        ...
        // 縦90度制限.
        if (m_Param.PanTilt.y > limitAngle)
        { m_Param.PanTilt.y = limitAngle; }
