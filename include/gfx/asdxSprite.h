...
struct UVRegion
{
    ...
    //! @brief      引数付きコンストラクタです.
    //-------------------------------------------------------------------------
    UVRegion(const Vector2& st0, const Vector2& st1)
    : uv0(st0)
    , uv1(st1)
    { /* DO_NOTHING */ }
    ...
    //! @brief      引数付きコンストラクタです.
    //-------------------------------------------------------------------------
    UVRegion(int x, int y, int w, int h, int texW, int texH)
    {
        auto fx = float(x) / float(texW);
        auto fy = float(y) / float(texH);
        auto fw = float(w) / float(texW);
        auto fh = float(h) / float(texH);
        uv0 = Vector2(fx, fy + fh);
        uv1 = Vector2(fx + fw, fy);
    }
};
...
public:
    ...
    //! @param[in]      h           スプライトの縦幅です.
    //! @param[in]      layer       レイヤーです(Z座標).
    //! @param[in]      region      テクスチャ座標範囲です.
    //-------------------------------------------------------------------------
    void Add(int x, int y, int w, int h, int layer, const UVRegion& region)
    { Add(x, y, w, h, layer, region.uv0, region.uv1); }
    ...
    { Add(x, y, w, h, 0, Vector2(0.0f, 0.0f), Vector2(1.0f, 1.0f)); }
    ...
    { Add(x, y, w, h, layer, Vector2(0.0f, 0.0f), Vector2(1.0f, 1.0f)); }
    ...
    //! @param[in]      region      テクスチャ座標範囲です.
    //-------------------------------------------------------------------------
    void Add(int x, int y, int w, int h, const UVRegion& region)
    { Add(x, y, w, h, 0, region.uv0, region.uv1); }
    ...
    //! @param[in]      region      テクスチャ座標範囲です.
    //-------------------------------------------------------------------------
    void Add(int x, int y, int w, int h, int layer, float rad, const UVRegion& region)
    { Add(x, y, w, h, layer, rad, region.uv0, region.uv1); }
