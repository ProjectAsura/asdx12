...
private:
    ...
    AllocationHolder            m_Holder;       //!< アロケーション情報.
...
public:
    ...
    //! @retval false   初期化に失敗.
    //-------------------------------------------------------------------------
    bool Init(uint64_t count, uint32_t stride, D3D12_RESOURCE_STATES state, bool dynamic = false);
    ...
    //! @retval false   初期化に失敗.
    //-------------------------------------------------------------------------
    bool Init(ID3D12GraphicsCommandList* pCmdList, uint64_t count, uint32_t stride, const void* pInitData, bool dynamic = false);
