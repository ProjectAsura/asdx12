...
public:
    ...
    enum TEXTURE_KIND
    {
        TEXTURE_BASE_COLOR,     //!< ベースカラーマップ.
        TEXTURE_NORMAL,         //!< 法線マップ.
        TEXTURE_ORM,            //!< オクルージョン/ラフネス/メタルネスマップ.
        TEXTURE_EMISSIVE,       //!< エミッシブマップ.

        MAX_TEXTURE_KIND        //!< テクスチャ種別の数.
    };
    using TextureArray = std::array<TextureHolder, MAX_TEXTURE_KIND>;
    ...
    //! @brief      テクスチャを取得します.
    //! 
    //! @param[in]      kind        テクスチャ種別
    //-------------------------------------------------------------------------
    const TextureHolder& GetTexture(TEXTURE_KIND kind) const;

    //-------------------------------------------------------------------------
    //! @brief      アルファモードを取得します.
    //! 
    //! @return     アルファモードを返却します.
    //-------------------------------------------------------------------------
...
private:
    ...
    TextureArray    m_Textures;         //!< テクスチャ.
    AlphaMode       m_AlphaMode;        //!< アルファモード.
    bool            m_TwoSided;         //!< 両面描画フラグ.
...
public:
    ...
    //! @param[out]     batchIndex      バッチ番号の格納先.
    //! @param[out]     instanceIndex   インスタンス番号の格納先.
    //! @retval true    検索ヒット.
    //! @retval false   見つからなかった.
    //-------------------------------------------------------------------------
    bool FindInstance(const char* name, uint32_t& batchIndex, uint32_t& instanceIndex) const;
