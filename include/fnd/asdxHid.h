...
public:
    ...
    //-------------------------------------------------------------------------
    void SetPlayerIndex(uint32_t index);
    ...
    //! @brief      バイブレーションさせます.
    //!
    //! @param[in]      lhs     左のモーターの振動量(有効値:0.0 ～ 1.0).
    //! @param[in]      rhs     右のモーターの振動量(有効値:0.0 ～ 1.0).
    //-------------------------------------------------------------------------
    void Vibrate(float lhs, float rhs) const;

    //-------------------------------------------------------------------------
    //! @brief      ボタンが押されたかチェックします.
    //!             フラグ切り替え向け用途として使用してください.
    //!
    //! @param[in]      buttonMask        チェックするボタン.
    //! @retval true    ボタンが押されました.
    //! @retval false   ボタンは押されません.
    //-------------------------------------------------------------------------
    bool IsDown(uint32_t buttonMask) const;
    ...
    //-------------------------------------------------------------------------
    bool IsHold(uint32_t buttonMask) const;
...
public:
    ...
    //-------------------------------------------------------------------------
    bool IsHold(uint32_t keyCode) const;
    ...
    //!             フラグの切り替え用途などに使用します.
    //-------------------------------------------------------------------------
    bool IsDown(uint32_t keyCode) const;
...
protected:
    ...
    //-------------------------------------------------------------------------
    static uint32_t ConvertKey(uint32_t keyCode);
...
public:
    ...
    //-------------------------------------------------------------------------
    bool IsHold(uint32_t button) const;
    ...
    //!             フラグの切り替え用途などに使用します.
    //-------------------------------------------------------------------------
    bool IsDown(uint32_t button) const;
    ...
    //-------------------------------------------------------------------------
    bool IsDrag(uint32_t button) const;
...
public:
    ...
    //! @brief      更新処理を行います.
    //! 
    //! @param[in]      windowHandle        ウィンドウハンドルです.
    //-------------------------------------------------------------------------
    ...
    //! @return     ゲームパッドを返却します.
    //-------------------------------------------------------------------------
    const GamePad& GetPad(uint32_t index) const;
...
private:
    ...
    // private variables.
    //=========================================================================
    static HidManager s_Instance;   //!< シングルトンインスタンスです.
    GamePad     m_Pad[4];           //!< ゲームパッドです.
    Mouse       m_Mouse;            //!< マウスです.
    Keyboard    m_Keyboard;         //!< キーボードです.
